...
def _get_cropped_images_pos(images_np, detector=None, pose=None):
    ...
    for index in range(len(isperson_dected)):
        if isperson_dected[index]:
            ...
            images_pose_points[images_pose_index] = (images_pose, pose_scores,images_np_cropped[index].shape[0:2])
    ...
    for i in range(1, batches):
        ...
        for index in range(len(isperson_dected_new)):
            if isperson_dected_new[index]:
                ...
                images_pose_points[images_pose_index] = (images_pose, pose_scores, images_np_cropped_new[index].shape[0:2])
            images_pose_index = images_pose_index+1
...
def process_video_with_index(video_index, enable_spectrogram = True):
    ...
    vidcap = cv2.VideoCapture('data/video_wav_indexed/video_{0}.mp4'.format(video_index))
    audio_rate, audio = wavfile.read('data/audio_wav_indexed/video_{0}.wav'.format(video_index))
...
def process_video_with_index_less_memory(video_index):
    ...
    min_avg_pose_score = .7
    vidcap = cv2.VideoCapture('data/video_wav_indexed/video_{0}.mp4'.format(video_index))
    print('data/video_wav_indexed/video_{0}.mp4'.format(video_index),'data/audio_wav_indexed/video_{0}.wav'.format(video_index))
    audio_rate, audio = wavfile.read('data/audio_wav_indexed/video_{0}.wav'.format(video_index))
    # if audio other than mono take 1st channel
    if (len(audio.shape)>1):
        audio = audio[:,0]
    ...
    for sec in range(1, num_secs+1):
        ...
        if len(video_images) == process_batch_len:
            ...
            for index in images_poses.keys():
                image_pose, score,dimension  = images_poses[index]
                score = score[0]
...
def main():    
    ...
    # sp = np.array([27,34,39])
    for video_index in range(start, num_videos+1):
        
        # if video_index not in sp:
        #     continue
