...
from model_utils import save_checkpoint, load_checkpoint,load_data,criterion,compute_loss
...
if gpu_cnt == 1:
    ...
    model = Nelson_model(audio_convout_size, CNNFeat, z_size, n_hidden, n_gaussians, n_layers).cuda()
elif gpu_cnt > 1:
    sys.stdout.write("More GPU's: {0}\n".format(gpu_cnt))
    model = torch.nn.DataParellel( Nelson_model(audio_convout_size, CNNFeat, z_size, n_hidden, n_gaussians, n_layers).cuda() )
else:
    sys.stdout.write("No GPU\n")
    model = Nelson_model(audio_convout_size, CNNFeat, z_size, n_hidden, n_gaussians, n_layers)
...
for epoch in range(start_epoch,epochs):
    ...
    for i, data in enumerate(dataloader1):
        ...
        loss = compute_loss(audio_inputs,next_steps,model,hidden)               
